...
// Only Chess related functions and variables.
...
var undo_stack = [];
let unstack = []; // stores same output as of undo_stack array but without any extra classes 

/*
 * undo the move user and bot has played.
 */
...
function undo() {
  ...
  board.position(game.fen()); // changes board's position.
}
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#undoBtn').on('click', function () {
    if (game.history().length >= 2) {
      const bm = botMoves.pop(); // Removes the last move from the botMoves array
      const um = userMoves.pop(); // Removes the last move from the userMoves array

      // Push removed moves into the unstack array
      unstack.push(bm);
      unstack.push(um);
      ...
      moveCount -= 2; // Decrement move count by 2
      // console.log(moveCount);

      // Remove highlights from the chessboard
      $board.find('.' + squareClass).removeClass('highlight-white');
      $board.find('.' + squareClass).removeClass('highlight-black');
      $board.find('.' + squareClass).removeClass('highlight-hint');

      // Undo twice: Opponent's latest move, followed by the player's latest move
      undo();
      window.setTimeout(() => {
        undo();
        window.setTimeout(() => {
          showHint(); // Show hint after undo actions
        }, 250);
      }, 250);
    }
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#redoBtn').on('click', function () {
    if (undo_stack.length >= 2) {
      let size = undo_stack.length; // Corrected `unstack` to `undo_stack`
      let uselen = size - 1;
      let botlen = size - 2;

      // Push moves into userMoves and botMoves arrays
      userMoves.push(undo_stack[uselen]);
      botMoves.push(undo_stack[botlen]);

      // console.log('Bot moves:', botMoves);
      // console.log('User moves:', userMoves);

      moveCount += 2; // Increment move count by 2
      // console.log(moveCount);
...
// suggests best legal move to the user
